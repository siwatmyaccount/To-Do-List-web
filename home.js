document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(()=> {
        ...
        document.getElementById('home').classList.add('active'); // เปิด Home
    }, 1000);
    setTimeout(() => {
        welcomeMsg.style.display = "none";
        // Fade-in Home
    ...
    sessionStorage.removeItem('username'); // ล้าง session ของ username

    // Path สำหรับ GitHub Pages
    const BASE_URL = "/To-Do-List-web/"; // <-- เปลี่ยนชื่อ myapp ให้ตรง repo ของคุณ
    window.location.href = BASE_URL + "index.html"; // ไปหน้า login จริงๆ
});
    ...
    function renderTasks(){
        ...
        tasks.forEach((t,i)=>{
            ...
            if(t.done){ text.style.textDecoration="line-through"; text.style.opacity="0.6"; }

            const status = document.createElement('span');
            status.style.marginLeft="10px"; status.style.fontWeight="600";
            status.textContent = t.done?"✅ Done":"⏳ Pending";

            const delBtn = document.createElement('button');
            delBtn.textContent = "Delete";
            delBtn.addEventListener('click', ()=>{
                tasks.splice(i,1); localStorage.setItem('tasks', JSON.stringify(tasks));
                renderTasks();
                showToast("Task deleted!","info");
            });

            li.appendChild(checkbox); li.appendChild(text); li.appendChild(status); li.appendChild(delBtn);
    ...
    loadBtn.addEventListener('click', ()=>{
        ...
        if(tasksVisible){ renderTasks(); loadBtn.textContent="Hide Tasks"; }
        else{ taskList.innerHTML=""; loadBtn.textContent="Show Tasks"; updateActivityStats(); }
    });
    ...
    document.getElementById('changePassBtn').addEventListener('click', ()=>{
        ...
        if(oldPass === "admin" && newPass.length >= 4){
